...
# endif
  ...
  struct flist **lst;   /* Pointer to link pointing here */
...
extern THREAD_LOCAL int verbose;             /* Report oddities in zip file structure */
...
extern THREAD_LOCAL int level;               /* Compression level */
...
#endif
/* 10/20/04 */
extern THREAD_LOCAL zoff_t dot_size;         /* if not 0 then display dots every size buffers */
extern zoff_t THREAD_LOCAL dot_count;        /* if dot_size not 0 counts buffers */
/* status 10/30/04 */
extern int display_counts;      /* display running file count */
extern int display_bytes;       /* display running bytes remaining */
extern THREAD_LOCAL int display_globaldots;  /* display dots for archive instead of for each file */
...
extern THREAD_LOCAL int noisy;               /* False for quiet operation */
...
#endif
extern THREAD_LOCAL int use_descriptors;     /* use data descriptors (extended headings) */
...
extern int THREAD_LOCAL mesg_line_started;    /* 1=started writing a line to mesg */
extern int logfile_line_started; /* 1=started writing a line to logfile */
extern THREAD_LOCAL char *key;               /* Scramble password or NULL */
extern char *tempath;           /* Path for temporary files */
extern THREAD_LOCAL FILE *mesg;              /* Where informational output goes */
